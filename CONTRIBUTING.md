...
6. For CI or hook changes, follow
   ...
   and run `./ci/verify policy` plus `./ci/verify security-fast`. Run
   `./ci/verify extended` for changes that affect the maintained Python quality,
   CI, dependency, documentation, release-evidence or protected-authority
   surfaces; the provider router applies the same high-risk classification.
