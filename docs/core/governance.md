...
This MkDocs page is a derived navigation projection. Canonical reusable rules
and workflows are:
...
- generic change-control policy in `core/change-control.yaml`;
- generic CI policy in `core/continuous-integration.yaml`;
- [toolkit self-policy](../../policy/change-control.yaml);
- [toolkit CI policy](../../policy/continuous-integration.yaml);
- [guardrail ownership and enforcement coverage](../../policy/guardrails.yaml).

Structure, IDs, links, inheritance and stable verification are CI gates.
Authority, semantic correctness and justified exceptions remain accountable
