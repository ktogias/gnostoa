...
| [#322](https://github.com/ktogias/gnostoa/issues/322), [#15](https://github.com/ktogias/gnostoa/issues/15) | [Documented convergence inference](../knowledge/assessments/pr-convergence-measurement.md): which minimum set of provider-observable flow and defect-density measures lets Gnostoa state whether a candidate is converging, and say when no verdict is possible? | The instrument set reproduces the PR #319 divergence from raw provider data and correctly declines to label the converged #312 and #314 as divergent; a real candidate is then blocked by a convergence question the existing #15 L0-lite workflow baseline cannot answer. Knowledge-only until that separate owner admission; it defines no gate, required check or merge authority. |
