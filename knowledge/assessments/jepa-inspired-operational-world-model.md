...
Before fixture or oracle exposure, the E0 preregistration must freeze a versioned source/fixture manifest. Every entry binds a stable pair ID; an immutable source revision or retained snapshot and canonical content digest, or a canonical generator version and seed; whether the pair tests discrimination or invariance; one primary distinction and any secondary tags; the expected oracle relation; and the compact-representation version. Mutable issue or pull-request pages alone are not fixture identities.

The minimum critical distinction matrix is:

| Domain | Required primary distinctions |
|---|---|
| Safety and observation | complete observation versus incomplete, stale, unavailable, or true absence; executed success versus skipped or missing work; identical duplicate versus conflicting duplicate |
| Authority | present versus missing authority; correct actor and scope versus mismatch; current authority versus expired or revoked authority |
| Subject | source or head identity; artifact tag versus immutable digest; evidence cut, review provenance, or provider generation |
| Effect applicability | selected and admitted versus merely proposed; unchanged versus changed intent, policy, or class; current versus stale target or precondition |

The frozen set must contain at least one discrimination pair for every listed leaf and at least one invariance pair for every domain. Mandatory slots are filled first; eligible fixtures within a slot and all declared extras are selected by lexical SHA-256 order of the canonical pair payload. Results cannot cause replacement. An unreproducible fixture, uncovered required leaf, or uncovered domain-level invariance yields no promotable E0 result; repair requires a new protocol version and a previously unexposed fixture cut.
...
Illustrative discovery pool, **not an enrolled corpus**: [#240](https://github.com/ktogias/gnostoa/issues/240) same-head/new-evidence behavior, [#257](https://github.com/ktogias/gnostoa/issues/257) rolling-trust provider generations, [#262](https://github.com/ktogias/gnostoa/issues/262)/[#272](https://github.com/ktogias/gnostoa/pull/272) normalization and candidate boundaries, and the source-bound [B2 recovery findings](https://github.com/ktogias/gnostoa/blob/e071ab60a418eddda5bf008004ee96faafbf1e7c/knowledge/assessments/b2-p2-fresh-session-and-effect-authority-findings.md). This proposal enrolls no sequence from that list. A future E1 preregistration must commit a versioned source manifest before fixture/oracle exposure; for every enrolled case it records the stable provider-object and event identities, immutable repository revision or retained provider snapshot, observed cut, extraction rule, and canonical content digest. Mutable issue or PR pages alone are insufficient. Do not invent timestamps or owner effort that were not recorded, and mark reconstruction with current knowledge as retrospective.
...
Before E1's frozen fixture/oracle exposure, its preregistration must freeze two executable reducers and record their versions independently. The **current/no-contract baseline** uses the existing typed projection and checks but has none of the proposed explicit expected-delta, invalidation, reconciliation, or mandatory-read-back transition contract. The **candidate reducer** adds that contract. Both are driven by the same ordered input stream, virtual time, and exogenous events and are scored against the same hidden terminal oracle; neither reducer receives the oracle.

The harness must reveal exactly one scheduled input at a time and wait for the reducer's response or checkpoint before revealing the next. Each reducer process may access only its current prefix. Future observations, actions, exogenous events, terminal labels, oracle data, and outcomes must be absent from its serialized input, mounted files, environment, logs, caches, and network-accessible context. Any prefix-visibility violation invalidates the experiment cut rather than counting as successful transition prediction.

#### Frozen E1 unit, oracle, and denominators

One **paired sequence** is the unit of analysis. Its versioned schema contains a stable `sequence_id`, source-manifest entry and canonical content digest, primary class, optional secondary tags, initial typed state and observation cut, ordered observations/actions/exogenous events, virtual-time schedule, maximum scheduled-input count, virtual deadline, protocol/harness/oracle versions, independently preregistered baseline and candidate reducer versions, authoritative terminal oracle, and secondary invariants.
...
A pair is mechanically valid only when the fixture parses and the harness supplies both reducers with the same canonical input digest, protocol and harness versions, prefix schedule, exogenous-event stream, and hidden oracle version; the manifest records the baseline reducer version and candidate reducer version separately. Reducer failure is a valid negative result. Source-manifest mismatch, prefix leakage, fixture corruption, or harness corruption invalidates the cut, remains visible in enrollment, and cannot be replaced after outcomes are seen.
...
The preregistration freezes the versioned source manifest and its provider-object/event identities, immutable revisions or retained snapshots, observed cuts, extraction rules and content digests, together with the canonical fixture generator and seed, class matrix, and selection manifest. Within each primary class, eligible fixtures are ordered by `(source_case_id, sequence_id)`; mandatory class slots are taken first, then any declared extra slots are filled by lexical SHA-256 order of the canonical fixture payload. There is no result-based replacement. If a source cannot be reproduced, or corruption leaves fewer than `N` valid pairs or any minimum uncovered, E1 produces no promotable result; repair requires a new protocol version and a previously unexposed cut.
...
The same preregistration must freeze the consequence-class schema, whether outcomes are mutually exclusive or multi-label, the safety-critical class designation, classwise one-versus-rest Brier components, calibration calculation, and positive-outcome coverage minima. Any class involving unauthorized, stale, or duplicate effects, false completion, or an ambiguous effect that could permit an unsafe retry is safety-critical. The provisional minimum is at least 5 positive outcomes for every declared class and at least 10 for every safety-critical class. Report enrollment count, prevalence, classwise Brier component, and calibration separately for every class. Classes cannot be pooled, relabelled, or removed after outcome exposure; inadequate class coverage yields no promotable result rather than an aggregate pass.
...
Before branch assignment, the E3 protocol must also freeze the minimum non-zero cost unit, the paired cost-difference calculation, the empirical high-cost quantile, and treatment of branches that do not reach a correct terminal disposition. An incomplete branch is charged the full frozen budget plus one minimum non-zero cost unit in the primary paired-cost analysis and is also counted as incomplete; it is never dropped from the denominator. The provisional maximum paired-regression margin is the larger of one minimum non-zero cost unit and 10 percent of that scenario's baseline cost. These defaults remain replaceable only before the E3 exposure boundary described below.
...
|---|---|---|---|---|
| E0 | Full source-labelled fixture oracle selected through the frozen manifest, critical distinction matrix, and deterministic rule above, compared with the proposed compact representation | Correct action-relevant discrimination or invariance for each paired fixture | 100% of required leaf-discrimination and domain-invariance pairs correct, and at least 95% correct overall on a frozen set of at least 30 pairs | Pass permits E1 fixture work; missing required coverage is no result, while any critical collision fails and requires representation repair and repeated noncritical aliasing stops compression of that field |
...
| E2 | Best preregistered rule, frequency, or nearest-case predictor using the same frozen pre-action fields, episode predicate, denominator, consequence schema, and evaluation cut | Aggregate and classwise one-versus-rest Brier score for the preregistered one-step consequence classes, with separately reported calibration | At least 15% lower paired aggregate Brier score than the best simple baseline, with a 90% bootstrap interval for the improvement above zero, on at least 50 scored episodes admitted by the frozen eligibility and missing-outcome rules; at least 5 positive outcomes per class and 10 per safety-critical class; no worse classwise Brier component or calibration for any safety-critical class; zero data-contract, authority, or information-leak violations | Pass permits an E3 admission proposal; an unmet sample, completeness, or class-coverage bound is no result, while aggregate failure, any safety-critical regression, or a failed effect or data boundary stops the branch |
| E3 | Best deterministic admissible-action ranking under the same frozen selection rule, action set, initial-state reset, versioned oracle, horizon, and cost schedule | Paired predeclared action/read-back cost to reach a correct terminal disposition, with the frozen failure penalty above | At least 15% lower median cost over at least 30 mechanically reset branchable scenarios or separately preregistered prospective pairs; no candidate pair exceeds its baseline cost by more than the frozen paired-regression margin; candidate empirical 95th-percentile cost does not exceed baseline; no reduction in completion rate; and zero authorization violations | Missing reset/oracle/cost equivalence is no result; any pairwise, tail-cost, completion, or authorization regression fails; a pass permits an E4 admission proposal only if a residual representation bottleneck is documented |
...
Subsequent exact-head PR review identified additional experiment-contract gaps and one provider-documentation disagreement. The following records both adopted repairs and the rejected finding:
...
| E0 had no deterministic fixture manifest or minimum distinction coverage | A curated 30-pair set could omit a material alias and still promote the representation | Added a versioned source/fixture manifest, twelve-leaf critical matrix, domain-level invariance controls, deterministic digest ordering, and a no-result rule for missing coverage |
| E2 used only an aggregate Brier improvement gate | Common outcomes could hide worse prediction or calibration for a rare safety-critical consequence | Added frozen class semantics, per-class positive-count minima, separate classwise scoring and calibration, and safety-critical non-regression gates |
...
| E3's median-only cost gate did not bound paired or tail regressions | A planner could become materially more expensive on a minority of scenarios while still passing the aggregate gate | Added a frozen failure penalty, maximum paired-regression margin, non-increasing empirical 95th-percentile cost, unchanged completion, and explicit fail semantics |
...
| The E1 discovery list was not an enrolled immutable source manifest | Mutable issue/PR content could silently change the replay basis | Marked the list illustrative and required every future enrolled case to bind stable provider/event identities, an immutable revision or retained snapshot, observed cut, extraction rule, and content digest |
| Reducers could appear to receive the full sequence and terminal oracle | Future observations or outcomes could leak into transition decisions | Required one-input-at-a-time prefix streaming, isolated reducer context, a hidden oracle, and cut invalidation on any prefix-visibility violation |
| Pair validity implied one shared reducer version | Distinct baseline and candidate reducers cannot truthfully share a version identity | Froze baseline and candidate reducer versions independently while requiring identical protocol, harness, prefix schedule, exogenous stream, and hidden oracle |
...
| A reviewer reported that GitHub Actions does not support `queue: single|max` | Removing a correct, date-bound provider capability would replace current primary-source evidence with stale reviewer knowledge | Not adopted: the official documentation read on 17 September 2026 explicitly defines both values, the 100-pending limit for `max`, and its incompatibility with `cancel-in-progress: true`; the paragraph remains bounded as provider documentation rather than executed characterization |
