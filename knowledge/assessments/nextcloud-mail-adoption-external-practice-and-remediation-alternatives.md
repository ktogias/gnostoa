...
|---|---|---|---|---|
| `S1` | [Diátaxis: How-to guides](https://diataxis.fr/how-to-guides/), live authoritative page | `2026-08-25` | A how-to is goal-oriented, follows the user's real task in a logical sequence, omits digression and links detail elsewhere. | General documentation method; not a Gnostoa contract. |
...
| `S11` | [Go Modules Reference](https://go.dev/ref/mod), current official language reference | `2026-08-25` | Vendoring records module versions in `vendor/modules.txt`, checks consistency with `go.mod`, and supports hash verification; rebuilding the vendor tree exposes local divergence. | Background for vendored-source manifests and verification. Go module semantics and destructive vendor regeneration do not transfer directly. |
...
|---|---|---|---|---|
| `P1` — start from one task-oriented adoption spine and link detail progressively | `D1`, `F1`, `R4`, bounded contribution to `H2` | Direct (`S1`, `S2`) | Put the first verified existing-project slice in goal order: select commitment, establish prerequisites and route, author the minimum, validate, generate context, classify the result. Link the full bootstrap/reference detail. | One golden path cannot decide whether the owner's ambiguous word “adopt” means evaluation or durable adoption; it must stop for `H2`, not choose. |
...
| `P9` — make source incorporation independently inspectable | `D4`, `F3`, `O1` | Direct for Git binding, background for vendoring (`S10`, `S11`) | A submodule gitlink can bind an exact commit; a vendor route should retain an exact source identity and a content manifest or equivalent comparison. | Git submodule, vendored copy and source-built install remain alternatives. Go's module manifest/checksum protocol must not be copied as a new Gnostoa dependency system. |

## What the research does and does not establish

The sources reinforce six direct correction targets already demonstrated by
Phase 1: task-oriented routing (`F1`), exact placement names (`F2`), explicit
