...
8. Give every candidate-owned Docker container and volume a validated unique name before creation and register that exact predeclared name as cleanup authority **before** the effect-capable create call. This preserves reconciliation authority even when Docker creates the resource but the CLI times out, disconnects or returns malformed stdout. Cleanup uses bounded retry/backoff and treats already-absent owned resources as reconciled; it never guesses a resource identity after an ambiguous create.
