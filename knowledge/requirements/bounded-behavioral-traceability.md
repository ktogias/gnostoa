...
readiness predicate or provider gate.
...
A trivial task outside those conditions records `NOT APPLICABLE` and one bounded
reason. It does not manufacture behavior rows merely to satisfy ceremony.

## Required workflow

### Executor checkpoint

For applicable work, record the compact map **before the first semantic
production mutation**. Each material behavior retains:
...
5. the corresponding test or other evidence, or an explicit reason it is
   unavailable;
6. the actual verification result;
7. whether the evidence `SUPPORTS`, `CONTRADICTS` or leaves the behavior
   `UNKNOWN`; and
8. separate executor and reviewer dispositions.

Bind the map to exact task, candidate and evidence identities. A relevant
mutation invalidates the affected row; reconcile it before reusing a result.
