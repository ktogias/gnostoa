...
The scheduled/manual extended suite now enforces Ruff formatting and an
explicit bounded lint set across the maintained Python tree, runs strict mypy
over `tools/` and `ci/`, adds branch-aware `tools/` coverage with an initial 65%
regression floor, performs known-vulnerability lookups for both exact Python
