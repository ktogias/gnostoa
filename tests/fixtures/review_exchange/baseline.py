"""Deliberately defective enumeration from activity, not independent obligations."""
...
def view(workspace: Path) -> dict:
    # Defect: a never-dispatched assignment is invisible to this denominator.
    ids = sorted(path.name for path in (workspace / "jobs").glob("*"))
