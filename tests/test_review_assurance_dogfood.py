...
def _dogfood_subject_observations() -> tuple[
...
def _current_advisory_documents() -> tuple[dict[str, object], dict[str, object]]:
    ...
    qualification.update(
        {
            "snapshot_id": "dogfood-pr239-unqualified",
            "revision": "1",
            "qualifying_authority": "unestablished-real-provider-characterization",
            "observed_at": "2026-09-12T00:00:00Z",
            "entries": [],
        }
    )
...
class ReviewAssuranceDogfoodTests(unittest.TestCase):
    ...
    def test_pr239_older_head_reviews_are_visible_and_non_authoritative(
        self,
    ) -> None:
        cases = _json(FIX / "cases.json")
        base = cases["base"]
    ...
    def test_pr239_current_advisory_dogfood_is_bootstrap_incomplete(self) -> None:
        input_document, policy_document = _current_advisory_documents()
