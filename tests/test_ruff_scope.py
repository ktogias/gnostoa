...
""",
    ...
    def test_style_surface_is_repository_root_scoped(self) -> None:
        ...
        self.assertIn("python -m ruff format --check --no-cache .", style)
        self.assertIn("python -m ruff check --no-cache .", style)
        self.assertIn("python -m ruff check --fix --no-cache .", style)
        self.assertIn("python -m ruff format --no-cache .", style)
        self.assertEqual(7, style.count("--no-cache"))
        ...
        safe_fix = fix_branch.index("python -m ruff check --fix --no-cache .")
        format_fix = fix_branch.index("python -m ruff format --no-cache .")
        format_check = fix_branch.index("python -m ruff format --check --no-cache .")
        lint_check = fix_branch.index("python -m ruff check --no-cache .")
