...
def _prepare_task(
    ...
    *,
    ...
    layout = {
        ...
        "qualification_base_evidence": task_root / "qualification" / "base" / "evidence",
        "qualification_base_tmp": task_root / "qualification" / "base" / "tmp",
        "qualification_reference_project": task_root / "qualification" / "reference" / "project",
        "qualification_reference_evidence": task_root / "qualification" / "reference" / "evidence",
        "qualification_reference_tmp": task_root / "qualification" / "reference" / "tmp",
    ...
    if observed_base != task.source.base_tree:
        blockers.append({"task": task.id, "code": "base-tree-identity-mismatch", "detail": f"declared {task.source.base_tree}, resolved {observed_base}"})
    ...
    if observed_reference != task.reference.tree:
        blockers.append({"task": task.id, "code": "reference-tree-identity-mismatch", "detail": f"declared {task.reference.tree}, resolved {observed_reference}"})
    base_dir = _materialize_verified(repo, task.source.base_tree, layout["subject"], "base", blockers, task.id)
    reference_dir = _materialize_verified(reference_repo, task.reference.tree, layout["subject"], "reference", blockers, task.id)
    ...
    if task.oracle_sha256 and task.oracle_sha256 != oracle_sha256:
        blockers.append({"task": task.id, "code": "oracle-identity-mismatch", "detail": f"declared {task.oracle_sha256}, observed {oracle_sha256}"})
    key_sha256: str | None = None
    if task.identification_key_path is not None:
        if not task.identification_key_path.is_file():
            blockers.append({"task": task.id, "code": "identification-key-missing", "detail": f"{task.identification_key_path} is not present"})
        else:
            key_sha256 = digest_path(task.identification_key_path)
            if task.identification_key_sha256 and task.identification_key_sha256 != key_sha256:
                blockers.append({"task": task.id, "code": "identification-key-identity-mismatch", "detail": "declared identification key digest does not match the file"})
    ...
    base_preparation = discover_preparation(base_dir, frozen_paths=_frozen_tree_paths(repo, task.source.base_tree))
    reference_preparation = discover_preparation(reference_dir, frozen_paths=_frozen_tree_paths(reference_repo, task.reference.tree))
    ...
    result = TaskResult(
        ...
        private_material={
            "oracle": {"sha256": oracle_sha256, "resolver": "owner-private", "locator": str(task.oracle_path)},
            **({"identification_key": {"sha256": key_sha256 or "", "resolver": "owner-private", "locator": str(task.identification_key_path)}} if task.identification_key_path is not None else {}),
        },
    )
    if preparation.required:
        _apply_task_preparation(task, result, repo, reference_repo, base_dir, reference_dir, blockers)
    adapter = adapters.get(task.adapter)
    staged_oracle_name = adapter.staged_oracle_name(oracle_sha256, task.oracle_path.name)
    ...
    for subject, source in (("base", base_dir), ("reference", reference_dir)):
        ...
        if occupant is not None and not stat.S_ISREG(occupant.st_mode):
            blockers.append({"task": task.id, "code": "oracle-staging-destination-occupied", "detail": f"the reserved qualification staging path {staged_oracle_name!r} exists in the subject and is not a regular file; refusing to follow or replace it"})
            continue
        if occupant is not None and destination.read_bytes() != oracle_bytes:
            blockers.append({"task": task.id, "code": "oracle-staging-destination-occupied", "detail": f"the reserved qualification staging path {staged_oracle_name!r} already exists in the subject with different bytes; refusing to overwrite subject content"})
            continue
    ...
    harness = adapter.build(task, workspace_path="/workspace", oracle_name=staged_oracle_name, test_config=test_config, projection=projection)
    blockers.extend(dict(item) for item in harness.blockers)
    result.harness = harness
    shape = read_shape(task.oracle_path)
    blockers.extend(qualify(shape, task.semantics, base_tree=base_dir, task_id=task.id, prior_qualification_sha256=task.prior_qualification_sha256))
    for relative, content in result.pending_execution_artifacts:
        result.stored_artifacts.append({"path": relative, "domain": profiles.EXECUTION, **_store_artifact(workspace, content)})
    for generated in harness.generated_files:
        result.stored_artifacts.append({"path": generated.path, "domain": profiles.QUALIFICATION, **_store_artifact(workspace, generated.content)})
        for destination in (layout["qualification_base_project"], layout["qualification_reference_project"]):
    ...
    for item in task.execution.read_only_inputs:
        ...
        if not item.source.is_file():
            blockers.append({"task": task.id, "code": "execution-input-unavailable", "detail": f"{item.id}: {item.source} is not present locally"})
            continue
        observed = digest_path(item.source)
        if item.sha256 and item.sha256 != observed:
            blockers.append({"task": task.id, "code": "execution-input-identity-mismatch", "detail": f"{item.id}: declared {item.sha256}, observed {observed}"})
            continue
        shutil.copy2(item.source, destination / item.source.name)
        result.execution_inputs.append({"id": item.id, "name": item.source.name, **_store_bytes(workspace, item.source.read_bytes())})
...
def _apply_task_preparation(
    ...
    if task.preparation_scheme == SCM_COMPATIBLE_SCHEME and task.runtime.preparation_tools:
        blockers.append({"task": task.id, "code": "preparation-tool-not-used-by-scheme", "detail": f"{SCM_COMPATIBLE_SCHEME} is a Gnostoa-owned algorithm and consumes no external artifact; declaring preparation_tools here would bind a producer that cannot change the produced bytes"})
        return
    ...
    for name in result.preparation.tool_names:
        ...
        if tool is None:
            blockers.append({"task": task.id, "code": "preparation-tool-undeclared", "detail": f"the frozen tree needs {name!r} to generate {list(result.preparation.generated_paths)}; declare it under runtime.preparation_tools with a locally available artifact"})
            continue
        artifact = Path(tool.artifact)
        if not artifact.is_file():
            blockers.append({"task": task.id, "code": "preparation-artifact-unavailable-offline", "detail": f"{tool.name} artifact {tool.artifact!r} is not present locally; offline preparation never acquires it"})
            continue
    ...
    if not task.preparation_scheme:
        blockers.append({"task": task.id, "code": "preparation-scheme-undeclared", "detail": "a build-generated artifact is required but no preparation.scheme is declared; the compiler does not choose a versioning scheme for you"})
        return
    receipts: list[PreparationReceipt] = []
    subjects = (
        ("base", base_dir, task.source.base_commit, repo, task.source.base_tree),
        ("reference", reference_dir, task.reference.commit or task.reference.tree, reference_repo, task.reference.tree),
    )
    ...
    try:
        for kind, subject_dir, commit, source_repo, subject_tree in subjects:
            ...
            generated_here = tuple(path for path in result.preparation.generated_paths if path not in frozen)
            ...
            receipts.append(apply_preparation(subject_dir, requirement, repository=source_repo, commit=commit, scheme=task.preparation_scheme, allow_existing=True))
    except PreparationError as exc:
        blockers.append({"task": task.id, "code": "preparation-failed", "detail": str(exc)})
        return
    for kind, subject_dir, _commit, source_repo, subject_tree in subjects:
        observed = _observed_tree(source_repo, subject_dir, ignore=list(per_subject_generated[kind]))
        if observed != subject_tree:
            blockers.append({"task": task.id, "code": "preparation-modified-tracked-source", "detail": f"{kind} tracked tree became {observed}, expected {subject_tree}"})
            return
    if receipts:
        ...
        for relative in per_subject_generated["base"]:
            ...
            if generated_file.is_file():
                result.pending_execution_artifacts.append((relative, generated_file.read_text()))
    result.prepared_runtime_identity = digest_of({"base_image": task.runtime.image, "preparation": result.preparation.as_json(), "scheme": task.preparation_scheme, "tools": available, "receipts": [receipt.as_json() for receipt in receipts]})


def _union_preparation(base: PreparationRequirement, reference: PreparationRequirement) -> PreparationRequirement:
    if not base.required and not reference.required:
        return base
    targets = list(base.generated_paths)
    targets += [path for path in reference.generated_paths if path not in base.generated_paths]
    ...
    return replace(primary, required=True, generated_paths=tuple(targets), inference=tuple(inference))
...
def _consume_certificates(spec: ExperimentSpec, blockers: list[dict[str, object]]) -> list[dict[str, object]]:
    reused: list[dict[str, object]] = []
    for request in spec.capabilities:
        if not request.certificate_path.is_file():
            blockers.append({"task": None, "code": "capability-certificate-missing", "detail": f"{request.capability}: {request.certificate_path} is not present"})
            continue
        observed = digest_path(request.certificate_path)
        if observed != request.certificate_sha256:
            blockers.append({"task": None, "code": "capability-certificate-identity-mismatch", "detail": f"{request.capability}: certificate file digest {observed} does not match the declared {request.certificate_sha256}"})
            continue
        try:
            certificate = certificates.load_file(request.certificate_path)
        except certificates.CertificateError as exc:
            blockers.append({"task": None, "code": "capability-certificate-invalid", "detail": f"{request.capability}: {exc}"})
            continue
        if certificate.satisfies(capability=request.capability, runtime_identity=request.required_runtime_identity, implementation_sha256=request.required_implementation_sha256, configuration_sha256=request.required_configuration_sha256, requested=request.requested_bounds):
            reused.append({"capability": request.capability, "certificate_sha256": request.certificate_sha256, "evidence_sha256": certificate.evidence_sha256, "requested_bounds": dict(request.requested_bounds), "reused": True})
        else:
            blockers.append({"task": None, "code": "capability-bounds-not-certified", "detail": f"{request.capability}: the certificate does not cover the declared identities or the requested {dict(request.requested_bounds)}; requalify or block"})
    return reused
...
def _deterministic_pre_effect_blocker(
    ...
    *,
    ...
    Tasks whose exact authorised disposition is prior-receipt reuse are excluded from
    that pass. The candidate-wide effect claim is created only after every fresh task
    has passed deterministic pre-effect viability, and remains immediately before the
    first actual ``qualify_subjects()`` effect.
    """
...
def prepare(
    ...
    *,
    ...
    with contextlib.ExitStack() as transaction_stack:
        if not offline:
            raise CompileError("online preparation is not implemented in v1; dependency acquisition is out of scope")
        root = Path(workspace)
        ...
        def finish(stage: str) -> PrepareResult:
            result.blockers = blockers
            result.stage = stage
            result.status = stages.READY_FOR_OWNER_REVIEW if stage == stages.READY_FOR_OWNER_REVIEW and not blockers else "BLOCKED"
            ...
            try:
                ...
                with retained_commit.coordination_lock(root):
                    if transaction_state.get("reserved"):
                        retained_commit.seal_reservation(root, expected_transaction_id=transaction_id, manifest_sha256=staged.manifest_sha256)
                    retained_commit.recover(root)
                    ...
                    if reservation is not None and reservation.transaction_id != transaction_id and retained_commit.owner_is_live(root, reservation):
                        return refuse_without_persisting(retained_commit.TRANSACTION_RESERVED, "another transaction holds the reservation for this workspace; refusing to persist over an in-flight transaction")
                    if _already_committed(current, staged):
                        pass
                    elif not held and _snapshot_identity(current) != _snapshot_identity(base_snapshot):
                        return refuse_without_persisting(retained_commit.CONCURRENT_STATE_CHANGED, "another invocation committed this workspace after the retained state was read; refusing to overwrite newer evidence with a stale snapshot")
                    else:
                        retained_commit.publish(root, staged=staged, generation=(current.generation + 1 if current else 1))
                    if held:
                        retained_commit.clear_reservation(root, expected_transaction_id=transaction_id)
        ...
        ledger.complete(stages.SEMANTIC_FROZEN, {task_id: digest_of(payload) for task_id, payload in semantic_inputs.items()})
        tasks = {task.id: _prepare_task(spec, task, root, blockers=blockers) for task in spec.tasks}
        result.tasks = tasks
        ledger.enter(stages.RUNTIME_PREPARED, {task.id: {"runtime": task.runtime.as_json(), "source": {"base_tree": task.source.base_tree, "reference_tree": task.reference.tree}, "preparation": tasks[task.id].preparation.as_json(), "scheme": task.preparation_scheme} for task in spec.tasks})
        if blockers:
            return finish(stages.RUNTIME_PREPARED)
        ledger.complete(stages.RUNTIME_PREPARED, {task_id: {"prepared_runtime_identity": task.prepared_runtime_identity, "preparation_receipt": task.preparation_receipt.identity if task.preparation_receipt else None} for task_id, task in tasks.items()})
        ledger.enter(stages.STATIC_QUALIFIED, {task.id: {"harness": _harness_json(tasks[task.id]), "oracle_sha256": tasks[task.id].oracle_sha256, "qualification_profiles": tasks[task.id].qualification_profiles, "execution_profile": tasks[task.id].execution_profile} for task in spec.tasks})
        ledger.complete(stages.STATIC_QUALIFIED, {task_id: task.capsule_identity for task_id, task in tasks.items()})
        ...
        for task in spec.tasks:
            ...
            try:
                ...
                blockers.append({"task": task.id, "code": "prior-qualification-receipt-invalid", "detail": str(exc)})
                continue
            if prior.backend != qualification_backend:
                blockers.append({"task": task.id, "code": "prior-qualification-backend-mismatch", "detail": f"the prior receipt was produced by backend {prior.backend!r}, but this request qualifies through {qualification_backend!r}; a receipt from another backend cannot stand in for it"})
                continue
            covers, mismatched = prior.covers(_qualification_bound(task, current))
            if not covers:
                blockers.append({"task": task.id, "code": "prior-qualification-receipt-not-current", "detail": f"the receipt does not bind {mismatched or 'a qualified outcome'}; requalify or remove the prior-qualification claim"})
                continue
        ...
        if blockers:
            return finish(stages.STATIC_QUALIFIED)
        candidate_sha256 = preflight_candidate_identity(experiment_id=spec.id, scope=BASE_REFERENCE_QUALIFICATION, qualification_backend=qualification_backend, tasks=tuple(candidate_tasks))
        result.preflight_candidate_sha256 = candidate_sha256
        authority_sha256 = digest_of(preflight_authority.as_json()) if preflight_authority is not None else None
        ...
        def arbitrate_transaction() -> PrepareResult | None:
            ...
            if authority_sha256 is not None:
                transaction_stack.enter_context(retained_commit.owner_liveness(root, transaction_id))
            for _ in range(_ARBITRATION_ROUNDS):
                try:
                    decision = retained_commit.arbitrate(root, transaction_id=transaction_id, base_identity=_snapshot_identity(base_snapshot), experiment_id=spec.id, scope=BASE_REFERENCE_QUALIFICATION, candidate_sha256=candidate_sha256, authority_sha256=authority_sha256)
                    ...
                    if decision.action == retained_commit.RECONCILE:
                        reload_retained()
                        continue
                    return refuse_without_persisting(retained_commit.TRANSACTION_RESERVED, decision.detail)
                except retained_commit.RetainedTransactionError as exc:
                    return refuse_without_persisting(exc.code, exc.detail)
            return refuse_without_persisting(retained_commit.INCONSISTENT_STATE, "the retained workspace kept changing under arbitration; refusing to act on a state that will not settle")
        ...
        if existing_qualification is not None and existing_qualification.complete:
            try:
                preserve_completed_candidate = retained_preflight.matching_completed_candidate_stage(root, ledger, candidate_sha256=candidate_sha256) is not None and retained_preflight.retained_lock_material_matches(root, experiment_id=spec.id, question=spec.question, claim_boundary=spec.claim_boundary, launch=spec.launch_payload(), capabilities=reused_certificates, artifact_store=str(root / "artifacts"))
        ...
        if preflight_authority is None:
            blockers.append({"task": None, "code": "base-reference-qualification-requires-preflight-authority", "detail": f"static preparation is complete; executing the declared oracle against BASE and REFERENCE requires an explicit owner preflight authority naming preflight candidate {candidate_sha256}"})
            return finish_authority_refusal()
        if not preflight_authority.covers(spec.id, BASE_REFERENCE_QUALIFICATION, candidate_sha256=candidate_sha256):
            if preflight_authority.experiment_id != spec.id or BASE_REFERENCE_QUALIFICATION not in preflight_authority.scope:
                blockers.append({"task": None, "code": "preflight-authority-out-of-scope", "detail": f"authority {preflight_authority.id!r} covers experiment {preflight_authority.experiment_id!r} scopes {list(preflight_authority.scope)}, not {BASE_REFERENCE_QUALIFICATION} for {spec.id!r}"})
            else:
                blockers.append({"task": None, "code": "preflight-authority-candidate-mismatch", "detail": f"authority {preflight_authority.id!r} approves preflight candidate {preflight_authority.preflight_candidate_sha256}, but this prepared request is {candidate_sha256}; the qualification inputs or backend changed since the authority was issued, so it is refused before any oracle runs"})
            return finish_authority_refusal()
        ...
        completed_qualification = retained_preflight.matching_completed_stage(ledger, qualification_stage_inputs)
        if completed_qualification is None:
            try:
                completed_qualification = retained_preflight.matching_completed_candidate_stage(root, ledger, candidate_sha256=candidate_sha256)
            except retained_preflight.RetainedPreflightError as exc:
                blockers.append({"task": None, "code": retained_preflight.INVALID_RETAINED_QUALIFICATION, "detail": str(exc)})
        ...
        if completed_qualification is not None:
            has_fresh_task = any(entry.get("qualification_mode") == "fresh" for entry in candidate_tasks)
            if has_fresh_task:
                try:
                    consumed = effect_claim.load_consumed_candidate(root, experiment_id=spec.id, scope=BASE_REFERENCE_QUALIFICATION, candidate_sha256=candidate_sha256, candidate_tasks=tuple(candidate_tasks))
                except effect_claim.EffectClaimError as exc:
                    blockers.append({"task": None, "code": exc.code, "detail": exc.detail})
                    return finish_without_persisting(stages.STATIC_QUALIFIED)
                if consumed.get("authority_sha256") != digest_of(preflight_authority.as_json()):
                    blockers.append({"task": None, "code": effect_claim.ALREADY_CONSUMED, "detail": f"preflight candidate {candidate_sha256} completed under a different authority; issuing another authority does not reopen or replace the retained transaction"})
                    return finish_without_persisting(stages.STATIC_QUALIFIED)
            try:
                restored = retained_preflight.load_completed_qualifications(root, candidate_sha256=candidate_sha256, stage_record=completed_qualification, task_ids=tuple(task.id for task in spec.tasks))
            except retained_preflight.RetainedPreflightError as exc:
                blockers.append({"task": None, "code": retained_preflight.INVALID_RETAINED_QUALIFICATION, "detail": str(exc)})
                return finish_without_persisting(stages.STATIC_QUALIFIED)
            for task in spec.tasks:
                ...
                if receipt.backend != qualification_backend:
                    blockers.append({"task": task.id, "code": retained_preflight.INVALID_RETAINED_QUALIFICATION, "detail": f"retained qualification backend {receipt.backend!r} does not match current backend {qualification_backend!r}"})
                    continue
                covers, mismatched = receipt.covers(_qualification_bound(task, current))
                if not covers:
                    blockers.append({"task": task.id, "code": retained_preflight.INVALID_RETAINED_QUALIFICATION, "detail": f"retained qualification is no longer current for {mismatched or 'the qualified outcome'}"})
                    continue
        ...
        if not qualification_reused:
            ...
            # Settle the deterministic viability of the whole exact fresh transaction
            # before the single candidate-wide irreversible claim. Reuse tasks are
            # zero-effect and deliberately excluded from fresh-only execution checks.
            for task in spec.tasks:
            ...
            for task in spec.tasks:
                ...
                outcome = qualify_subjects(
                    ...
                    reference_tree=current.qualification_paths.get("reference", reference_path),
                    ...
                    bound=bound,
                    expectations={"base": task.expectations.base, "reference": task.expectations.reference},
                ...
                if not outcome.qualified:
                    blockers.append({"task": task.id, "code": "base-reference-qualification-failed", "detail": f"base {outcome.base.classification}: {outcome.base.detail}; reference {outcome.reference.classification}: {outcome.reference.detail}"})
            if blockers:
                return finish(stages.STATIC_QUALIFIED)
            ledger.complete(stages.BASE_REFERENCE_QUALIFIED, {task_id: task.qualification.identity for task_id, task in tasks.items() if task.qualification})

        ledger.enter(stages.BOUNDARY_QUALIFIED, {"certificates": reused_certificates, "resources": spec.resources.as_json()})
        ledger.complete(stages.BOUNDARY_QUALIFIED, {"certificates": len(reused_certificates)})
        for task in spec.tasks:
            if not task.execution.command:
                blockers.append({"task": task.id, "code": "execution-command-required-for-freeze", "detail": "a lock reported READY_FOR_OWNER_REVIEW must already be executable; declare execution.command, which is never inferred from the qualification invocation"})
        if blockers:
            return finish(stages.BOUNDARY_QUALIFIED)
        if spec.arms and not spec.assignment.schedule:
            blockers.append({"task": None, "code": "assignment-schedule-required", "detail": "arms are declared but experiment.assignment carries no ordered schedule; declare assignment.schedule inline or bind assignment.schedule_artifact with its sha256. The compiler never derives assignment order."})
            return finish(stages.BOUNDARY_QUALIFIED)
        scheduled_arms = list(spec.assignment.arms) or ["default"]
        schedule = [entry.as_json() for entry in spec.assignment.schedule] or [{"task": task.id, "repetition": repetition, "arm": "default"} for task in spec.tasks for repetition in range(1, spec.assignment.repetitions + 1)]
        schedule_reasons = runplan.validate_schedule(schedule=schedule, task_ids=[task.id for task in spec.tasks], arms=scheduled_arms, repetitions=spec.assignment.repetitions)
        if schedule_reasons:
            blockers.append({"task": None, "code": "assignment-schedule-not-a-complete-permutation", "detail": "the schedule must be exactly one permutation of tasks x repetitions x arms: " + "; ".join(schedule_reasons)})
            return finish(stages.BOUNDARY_QUALIFIED)
        plan = runplan.compile_plan(schedule=schedule, schedule_source=spec.assignment.source, schedule_sha256=spec.assignment.schedule_sha256, arm_inputs={arm.name: [{"id": item.id, "sha256": item.sha256 or "", "source": str(item.source)} for item in arm.inputs] for arm in spec.arms})
        result.run_plan = plan
        ledger.enter(stages.EXECUTION_FROZEN, {"capsules": {task_id: task.capsule_identity for task_id, task in tasks.items()}, "launch": spec.launch_payload(), "run_plan": plan.identity})
        experiment_lock = lock_module.build(experiment_id=spec.id, question=spec.question, claim_boundary=spec.claim_boundary, launch=spec.launch_payload(), tasks=[task.as_json() for task in tasks.values()], capabilities=reused_certificates, stage_receipts=ledger.receipts(), authority=preflight_authority.as_json(), run_plan=plan.as_json(), artifact_store=str(root / "artifacts"))
        try:
            transaction_state["staged_lock"] = experiment_lock.stage(root)
        except lock_module.LockError as exc:
            blockers.append({"task": None, "code": "experiment-lock-conflict", "detail": str(exc)})
        ...
        if missing:
            blockers.append({"task": None, "code": "readiness-missing-stage-receipts", "detail": f"no completed receipt for {missing}"})
            return finish(stages.EXECUTION_FROZEN)
        ledger.enter(stages.READY_FOR_OWNER_REVIEW, {"lock_sha256": experiment_lock.identity})
        ledger.complete(stages.READY_FOR_OWNER_REVIEW, {"lock_sha256": experiment_lock.identity})
        return finish(stages.READY_FOR_OWNER_REVIEW)


def _snapshot_identity(snapshot: retained_commit.CommittedSnapshot | None) -> str | None:
    return snapshot.identity if snapshot is not None else None


def _already_committed(current: retained_commit.CommittedSnapshot | None, staged: retained_commit.StagedTransaction) -> bool:
    if current is None:
        return False
    return current.stages_sha256 == staged.stages_file_sha256 and current.state_sha256 == staged.state_file_sha256 and (staged.lock_file_sha256 is None or current.lock_file_sha256 == staged.lock_file_sha256)
...
def _state_payload(result: PrepareResult) -> str:
    payload = {
        ...
        retained_commit.TRANSACTION_MARKER_FIELD: retained_commit.TRANSACTION_MARKER_VALUE,
    }
...
def status(workspace: str | Path) -> dict[str, Any]:
    ...
    for _ in range(_STATUS_READ_ATTEMPTS):
        ...
        refusal = retained_commit.RetainedTransactionError(retained_commit.INCONSISTENT_STATE, "the retained workspace kept changing while it was being read; refusing to report a state no committed snapshot vouches for")
    if refusal is not None:
        payload["status"] = "BLOCKED"
        payload["blockers"] = [*payload.get("blockers", []), {"task": None, "code": refusal.code, "detail": refusal.detail}]
    return payload
