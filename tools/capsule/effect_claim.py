...
def _disposition_summary(
    ...
    if not summary or not any(item["qualification_mode"] == "fresh" for item in summary):
        raise EffectClaimError(
            INVALID_CLAIM,
...
def _open_visible_directory(path: Path, *, label: str) -> int:
    ...
    if not stat.S_ISDIR(identity.st_mode) or not _same_object(observed, identity):
        os.close(descriptor)
        raise EffectClaimError(INVALID_CLAIM, f"{label} namespace changed while opening")
    return descriptor
...
def _read_existing_claim(directory_fd: int, name: str) -> bytes | None:
    try:
        ...
        raise EffectClaimError(INVALID_CLAIM, f"cannot inspect existing claim: {exc}") from exc
    ...
    try:
        ...
        while True:
            ...
            if total > _MAX_CLAIM_BYTES:
                raise EffectClaimError(INVALID_CLAIM, "the candidate claim is oversized")
...
def _validate_existing_claim(
    ...
    *,
    ...
    try:
        ...
        raise EffectClaimError(INVALID_CLAIM, f"candidate claim is not valid JSON: {exc}") from exc
    ...
    if set(payload) != expected_keys:
        raise EffectClaimError(INVALID_CLAIM, "candidate claim has unexpected or missing fields")
    ...
    if payload.get("schema") != CLAIM_SCHEMA:
        raise EffectClaimError(INVALID_CLAIM, "candidate claim uses an unsupported schema")
    if (
        payload.get("experiment_id") != experiment_id
        or payload.get("scope") != scope
        or payload.get("preflight_candidate_sha256") != candidate_sha256
    ):
        raise EffectClaimError(INVALID_CLAIM, "candidate claim identity does not match its path")
    if payload.get("qualification_disposition") != list(disposition):
        raise EffectClaimError(INVALID_CLAIM, "candidate claim disposition does not match")
    ...
    if payload.get("authority_sha256") != digest_of(authority_dict):
        raise EffectClaimError(INVALID_CLAIM, "candidate claim authority digest does not match")
    try:
        authority = parse(authority_dict)
    except ValueError as exc:
        raise EffectClaimError(INVALID_CLAIM, f"candidate claim authority is invalid: {exc}") from exc
    if not authority.covers(experiment_id, scope, candidate_sha256=candidate_sha256):
        raise EffectClaimError(INVALID_CLAIM, "candidate claim authority does not cover the claim")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                raise EffectClaimError(
                    INVALID_CLAIM, "candidate claim appeared concurrently but cannot be trusted"
                )
            except OSError as exc:
                raise EffectClaimError(
            ...
            try:
                ...
                raise EffectClaimError(
                    WRITE_FAILED, f"cannot durably retain candidate claim directory entry: {exc}"
                ) from exc
        finally:
            os.close(claim_fd)
    finally:
