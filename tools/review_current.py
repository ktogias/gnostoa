...
def _run_docker(
    ...
    *,
    ...
    try:
        ...
        raise ProtectedJudgeUnavailable(f"protected Docker execution failed: {exc}") from exc
    if len(result.stdout) > _MAX_RUNTIME_OUTPUT_BYTES:
        raise ProtectedJudgeUnavailable("protected Docker stdout exceeds the bounded size")
    if len(result.stderr) > _MAX_RUNTIME_OUTPUT_BYTES:
        raise ProtectedJudgeUnavailable("protected Docker stderr exceeds the bounded size")
    return result
...
def _decode_result(raw: bytes) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ProtectedJudgeUnavailable("prior-integrated judge result must be an object")
    return value
...
def run_prior_integrated_judge(
    *,
    ...
    if _SHA40.fullmatch(expected_revision) is None:
        raise ProtectedJudgeUnavailable("protected judge revision is not an exact Git commit")
    if _SHA256.fullmatch(expected_surface_digest) is None:
        raise ProtectedJudgeUnavailable("protected judge public-surface digest is invalid")
    ...
    with tempfile.TemporaryDirectory(prefix="gnostoa-r2a-judge-", dir="/tmp") as directory:
        root = Path(directory)
