...
def effective_policy_issues(policy: object) -> list[str]:
    ...
    if requirement == "required":
        ...
        required_sources = (
            collection.get("required_sources") if isinstance(collection, dict) else None
        )
        capabilities = (
        ...
        minimum = (
            quorum.get("minimum_distinct_domains") if isinstance(quorum, dict) else None
        )
...
def _load_source(path: Path, stack: tuple[Path, ...]) -> dict[str, Any]:
    ...
    if not isinstance(extends, list) or len(extends) > 1:
        raise KnowledgeFormatError(
            f"Review-policy extends must contain at most one parent in {resolved}"
        )
    merged: dict[str, Any] = {}
    for reference in extends:
        if not isinstance(reference, str):
            raise KnowledgeFormatError(
                f"Review-policy parent reference must be a string in {resolved}"
            )
...
def resolve_project_policy(path: Path, change_class: str) -> dict[str, Any]:
    ...
    if not isinstance(defaults, dict) or not isinstance(classes, dict):
        raise KnowledgeFormatError(
            "Review-policy source must define mapping defaults and change_classes"
        )
