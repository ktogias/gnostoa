...
class ExecutionObservation:
    """Provider-neutral observation with controller-derived execution bindings.
    ...
    Command, effective-limit, backend and runtime identities are captured by the
    controller so later composition cannot bind a capture to caller-declared
    execution metadata that did not produce it.
    """
    ...
    def __post_init__(self) -> None:
        _need(
            _SHA256_RE.fullmatch(self.command_sha256) is not None, "OBSERVATION_BINDING"
        )
        _need(
            _SHA256_RE.fullmatch(self.limits_sha256) is not None, "OBSERVATION_BINDING"
        )
        for identity in (self.backend_identity, self.runtime_identity):
            _need(
                isinstance(identity, str)
                and 0 < len(identity) <= 1024
...
def _command_binding(command: Sequence[str]) -> str:
    return _binding_digest({"argv": list(command)})


def _limits_binding(limits: ExecutionLimits) -> str:
...
def _backend_runtime_identity(
    backend: ExecutionBackend, command: Sequence[str]
) -> tuple[str, str]:
    backend_type = type(backend)
    type_identity = f"{backend_type.__module__}.{backend_type.__qualname__}"
    if backend_type is DockerBackend:
        docker = cast(DockerBackend, backend)
        return "vf0-docker/v1", f"oci-image:{docker.image}"
    if backend_type is SubprocessBackend:
        executable = command[0]
        command_runtime = (
            f"subject-path:{executable}"
            if executable.startswith("./")
            else f"host-path:{executable}"
        )
        return (
            "vf0-local-unshare/v1",
            "local-unshare:"
            + _binding_digest(
                {
    ...
    return (
        f"python-backend:{type_identity}",
        f"opaque-python-runtime:{type_identity}",
    )
...
def execute(
    ...
    command_sha256 = _command_binding(command_snapshot)
    limits_sha256 = _limits_binding(chosen_limits)
    backend_identity, runtime_identity = _backend_runtime_identity(
        backend, command_snapshot
    )
